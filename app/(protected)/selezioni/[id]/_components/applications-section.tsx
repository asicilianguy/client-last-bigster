...
export function ApplicationsSection({ selectionId }: { selectionId: number }) {
  const { data, error, isLoading } =
    useGetApplicationsBySelectionIdQuery(selectionId);

  return (
    <Card className="shadow-sm border-0 h-full mt-4 relative">
            ...
            {data.data.length > 0 ? (
                    ...
                    <TableRow key={application.id}>
                      <TableCell>
                        <div className="flex items-center gap-3">
                          <Avatar>
                            <AvatarImage
                              src={`https://avatar.vercel.sh/${application.email}.png`}
                            />
                            <AvatarFallback>
              ...
              <div className="flex flex-col items-center justify-center rounded-lg border-2 border-dashed py-12 text-center">
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Non sono ancora state ricevute candidature per questa
                  selezione.
                </p>
              </div>
            )}
