...
export default function SelezioniDashboardPage() {
  ...
  const filteredSelections = data?.data.filter((selection: any) => {
    if (isCEO || isDeveloper) {
      // CEO and Developers see all selections
      return true;
    }
    if (isResponsabile) {
      // Responsabili see selections for their department
      return selection.responsabile_id === user?.id;
    }
    if (isHR) {
      // HR sees selections they're assigned to
      return selection.risorsa_umana_id === user?.id;
    }
    // Default fallback
    return false;
  });
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            {isCEO && "Approva e gestisci tutte le selezioni aziendali."}
            {isResponsabile &&
              "Visualizza e gestisci le selezioni del tuo reparto."}
            {isHR &&
              "Gestisci gli annunci e le candidature delle selezioni assegnate."}
            {!isCEO &&
              !isResponsabile &&
              !isHR &&
              "Visualizza e gestisci tutte le selezioni in corso."}
          </p>
        </div>
        {canCreateSelection && (
          <Button
      ...
      >
        {filteredSelections && filteredSelections.length > 0 ? (
          filteredSelections.map((selection: any) => (
            <motion.div key={selection.id} {...{variants: itemVariants as any}}>
                ...
                <CardFooter className="bg-muted/50 p-4">
                  ...
                  >
                    <Link href={`/selezioni/${selection.id}`}>
                      <span>
                        {isCEO && selection.stato === "CREATA" && "Approva"}
                        {isCEO &&
                          selection.stato === "APPROVATA" &&
                          !selection.risorsa_umana_id &&
                          "Assegna HR"}
                        {isHR && "Gestisci"}
                        {isResponsabile && "Visualizza"}
                        {!isCEO && !isHR && !isResponsabile && "Gestisci"}
                      </span>
                      <ArrowRight />
                    </Link>
                  </Button>
